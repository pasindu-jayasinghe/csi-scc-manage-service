...
import { DistanceBaseDto, FuelBaseDto, PassengerOffroadDto } from 'src/emission/calculation/dto/passenger-offroad.dto';
...
import { Ownership } from '../../../../../project/enum/ownership.enum'
...
@Injectable()
export class PassengerOffroadService extends TypeOrmCrudService<PassengerOffroadActivityData> implements ExcellUploadable , BulckUpdatable, ProgressRetriever, ExcelDownloader{

  getDto() {
    return new PassengerOffroadActivityData();
  }

  constructor(
    @InjectRepository(PassengerOffroadActivityData) repo,
  ...
  async getProgressData(projectId: number, unitIds: number[]) {
    ...
    let emissionSource = sourceName.passenger_offroad
    let parameters = [
      { name: 'Vehicle No', code: 'vehicleNo' }
    ]
      ...
      for await (let key of Object.keys(activityDataUnit)) {
        ...
        if (pues && pues.isComplete) {
          response.push({
            ...
            esName: 'Passenger Offroad - ' + ownership,
  ...
  async bulkDelete(ids: number[], isPermant: boolean) {
   ...
   async bulkCalculate(unitIds: number[], projectId: number) {
    return await this.emissionSourceRecalService.bulkCalculate(this, unitIds, projectId, this.repo, sourceName.passenger_offroad);
  }
  ...
  async create(createDto: PassengerOffroadActivityData) {

    let {fuel, distance} = this.getData(createDto)
     
    const calculationData: PassengerOffroadDto = {
    ...
    const emission = await this.calculationService.calculate({
      sourceName: sourceName.passenger_offroad,
  ...
  async update(id: number, updateDto: PassengerOffroadActivityData) {
  ...
  async getBaseData(dto: PassengerOffroadActivityData): Promise<BaseDataDto> {
    ...
    req.sourceName = sourceName.passenger_offroad
  ...
  getData(dto: PassengerOffroadActivityData) {
    ...
    if (dto.method === TransportMode.fuel_base) {
      ...
      distanceBaseData.distance = dto.totalDistanceTravelled
      distanceBaseData.distance_unit = dto.totalDistanceTravelled_unit
      distanceBaseData.fe = dto.fuelEconomy
      distanceBaseData.fe_unit = dto.fuelEconomy_unit
      distanceBaseData.trips = dto.noOfTrips
      distanceBaseData.twoWay = dto.option === TransportOption.two_way ? true : false
    }
  ...
  async updateTotalEmission(dto: PassengerOffroadActivityData, calData: PassengerOffroadDto, emission: any){
    ...
    await this.pesService.addEmission(reqPes)
    ...
    await this.projectService.addEmission(reqProject)
  }
  ...
  addFromExcell(unit: Unit, project: Project, user: User, data: any, variable_mapping: any[], year: number) {
    let units = this.parameterUnit.passenger_offroad_units;
    let types = this.parameterUnit.fuelType;
    let strokeTypes = this.parameterUnit.strokes;

    let dto = new PassengerOffroadActivityData();
    dto['unit'] = unit;
    dto['project'] = project;
    dto['user'] = user;
    ...
    if (data['stroke'] === 'Four Stroke'){
      dto.stroke = (strokeTypes.find(s => s.id === 2)).code;
    }else if (data['stroke'] === 'Two Stroke'){
      dto.stroke = (strokeTypes.find(s => s.id === 1)).code;
    }else {
      dto.stroke = dto.stroke;
    }
    ...
    dto.totalDistanceTravelled_unit = units.distance[0].code;
