...
import { DistanceBaseDto, FuelBaseDto, PassengerRailDto } from 'src/emission/calculation/dto/passenger-rail.dto';
...
@Injectable()
export class PassengerRailService extends TypeOrmCrudService<PassengerRailActivityData> implements ExcellUploadable , BulckUpdatable, ProgressRetriever, ExcelDownloader{

  getDto() {
    return new PassengerRailActivityData();
  }

  constructor(
    @InjectRepository(PassengerRailActivityData) repo,
  ...
  async getProgressData(projectId: number, unitIds: number[]) {
    ...
    let emissionSource = sourceName.passenger_rail
    let parameters = [
      { name: 'Vehicle No', code: 'vehicleNo' }
    ]
      ...
      for await (let key of Object.keys(activityDataUnit)) {
        ...
        if (pues && pues.isComplete) {
          response.push({
            ...
            esName: 'Passenger Rail - ' + ownership,
  ...
  async bulkDelete(ids: number[], isPermant: boolean) {
   ...
   async bulkCalculate(unitIds: number[], projectId: number) {
    return await this.emissionSourceRecalService.bulkCalculate(this, unitIds, projectId, this.repo, sourceName.passenger_rail);
  }
  ...
  async create(createDto: PassengerRailActivityData) {
    let {fuel, distance} = this.getData(createDto)
    const calculationData: PassengerRailDto = {
    ...
    const emission = await this.calculationService.calculate({
      sourceName: sourceName.passenger_rail,
    ...
    return await this.passengerRailRepository.save(createDto);
  }

  findAll() {
    return this.passengerRailRepository.find();
  }

  async update(id: number, updateDto: PassengerRailActivityData) {
  ...
  async getBaseData(dto: PassengerRailActivityData): Promise<BaseDataDto> {
    ...
    req.sourceName = sourceName.passenger_rail
  ...
  getData(dto: PassengerRailActivityData) {
  ...
  async updateTotalEmission(dto: PassengerRailActivityData, calData: PassengerRailDto, emission: any){
  ...
  addFromExcell(unit: Unit, project: Project, user: User, data: any, variable_mapping: any[], year: number) {
    ...
    let dto = new PassengerRailActivityData();
    dto['unit'] = unit;
    dto['project'] = project;
    dto['user'] = user;
